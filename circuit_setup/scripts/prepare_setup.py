...
def usage():
    ...
    print("Usage:")
    print(
        "\t./" + os.path.basename(sys.argv[0]) + " <config file> <circom output file>"
    )
...
""".format(
                    name=name,
                    claim_template_len=len(claim_template),
            ...
            if (
                config[name].get("reveal_bytes") is not None
                and config[name]["reveal_bytes"] == True
            ):
                if config[name].get("max_claim_byte_len") is not None:
                    if (config[name]["max_claim_byte_len"] % MAX_FIELD_BYTE_LEN) != 0:
                        print(
                            "max_claim_byte_len must be a multiple of MAX_FIELD_BYTE_LEN"
                        )
                        sys.exit(-1)
...
""".format(name=name, reveal_function=reveal_function, is_number=is_number)
            ...
            ###  begin reveal hashed
                ...
                f.write(
                    """    var {}_max_claim_byte_len = {};""".format(
                        name, config[name]["max_claim_byte_len"]
                    )
                )
...
""".format(name=name, is_number=is_number)
            ...
            ### begin timing-channel gadget (weighted padding on a hidden claim)
            elif config[name].get("timing_gadget") is not None:
                gadget = config[name]["timing_gadget"]
                gadget_weight = gadget["weight"]
                gadget_n = gadget["n"]
                gadget_max_len = config[name].get(
                    "max_claim_byte_len", MAX_FIELD_BYTE_LEN
                )
                # We read the claim value as a number, but keep it private (so we
                # do not add it to public_inputs). Then we use the thermometer
                # with s = value * weight, so maybe the proving time leaks the value
                # over the witness density.
                f.write(
                    """
    // timing-channel gadget on hidden claim "{name}"
    var {name}_max_claim_byte_len = {max_len};
    component reveal_{name} = RevealClaimValue(max_json_bytes, {name}_max_claim_byte_len, field_byte_len, 1); // last param = 1 means it's a number
    // claim is a jwt bytes string so we have to figure out where our value is listed.
    reveal_{name}.json_bytes <== jwt_bytes;
    // l and r are the boundaries, where the value starts and ends. The value is everything between them.
    // if the age is 25 e.g. this will be value_l = 2, value_r = 5, interpret as a number so 25.
    reveal_{name}.l <== match_{name}_name.value_l;
    reveal_{name}.r <== match_{name}_name.value_r;

    signal {name}_S;
    {name}_S <== reveal_{name}.value * {weight};
    component {name}_thermo = Thermometer({gadget_n});
    {name}_thermo.s <== {name}_S;
""".format(name=name, max_len=gadget_max_len, weight=gadget_weight, gadget_n=gadget_n)
                )
            ### end timing-channel gadget
...
""".format(
                ...
                max_msg_len=config["max_cred_len"],
                max_json_len=base64_decoded_size(config["max_cred_len"]),
                max_field_byte_len=MAX_FIELD_BYTE_LEN,
                limb_size=limb_size,
                n_limbs=n_limbs,
            )
        )
